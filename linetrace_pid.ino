...
float vel_target = 0.1, // ここの値は、初期デューティー比の時の平均速度とする
      L_sen_target = 540,  // センサ発光：700, センサ消灯：300 (0~1023)  黒：発光、白：消灯
      R_sen_target = 540;
...
float L_sen_integral = 0,
      R_sen_integral = 0,
      vel_integral = 0;

float v_pid=0, L_pid=0, R_pid=0;
float L_old_pid=0, R_old_pid=0;
...
float v_PID_Control()
{
  ...
  old_vel_diff = vel_diff;

  vel_diff = v - vel_target;

  vel_integral += (old_vel_diff + vel_diff) / 2 * dt;
...
void loop()
{
  ...
  L_val = L_old_pid + (L_pid - L_old_pid);
  R_val = R_old_pid + (R_pid - R_old_pid);
  // analogWrite(6, map(L_val, -10000, 10000, 0, 55)); 
  // analogWrite(7, map(R_val, -10000, 10000, 0, 55));
  analogWrite(6, constrain(L_val, -55, 55));  // -が前進になってない？
  analogWrite(7, constrain(R_val, -55, 55));
